 ...
 * @since 1.0
 */
public class PDFGVTBuilder extends GVTBuilder {
	 ...
	 */
	public PDFGVTBuilder(final boolean forceVector) {
	...
	protected void buildComposite(final BridgeContext ctx, final Element e,
		...
		for (final var obj : cgn) {
			if (!(obj instanceof final GraphicsNode gn)) {
				continue;
			}
			final var c = gn.getComposite();
			...
			gn.setFilter(new PDFTransparencyRable(gn, this.forceVector));
		}
	}
}
